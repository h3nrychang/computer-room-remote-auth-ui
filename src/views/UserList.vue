...
<!--          <el-button @click="editUser(row)" size="mini" type="primary"><el-icon><EditPen /></el-icon>编辑</el-button>-->
  ...
  <!-- 详细信息 -->
  <el-dialog v-model="signupDialogVisible" title="新建用户（注册用户）" :width="dialogWidth" :close-on-click-modal="false">
    <el-form :model="formData" ref="form" label-width="100px" :rules="formRules" @submit.prevent="submitForm">
    ...
    <template #footer>
      <el-button @click="signupDialogVisible = false">关闭</el-button>
  ...
  if (form) {
    ...
    form
        ...
        .then(async () => {
          try {
            // 请求 API 注册
            const response = await axios.post('/users/create', formData.value, {
              headers: {
            ...
            signupDialogVisible.value = false; // 关闭对话框
            formData.value = { username: '', password: '', telephone: '' }; // 清空表单数据
